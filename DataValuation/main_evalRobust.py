...
from core.dataloader import construct_dataowner, sample_from_dataowner
from core.proxy_models import cnn_data_to_acc, logistic_data_to_acc
from utils.utils import model_name, collect_from_loader, seed_everything
from utils.preprocess import noise, RandomizedResopnse, attack_pgd
from scripts.experimental_setting import experimental_setting
from train.train_uf import train_accuracy_line
from scripts.sample_utility import sample_utility, sample_utility_unbalance_owner
from train.train_adda import train_adda_uf
from train.train_uf import train_uf
from train.train_uf_y import train_uf_y
from train.train_src_net import pretraining_from_data, train_source_from_loader
from models.models import get_model, model_for_cifar10, model_for_mnist
from core.shapley import get_shapley
    ...
    if os.path.exists(ext_file) and os.path.exists(ds_file):
        ...
        if src is not None:
            ...
            extractor, deepset = train_uf(tgt, utility_data[0], train_set, test_set, args.epochs,
                                          ...
                                          lr_extractor=args.lr_extractor, lr_deepsets=args.lr_deepsets,device='cuda')
        extractor.eval()
        deepset.eval()
...
if __name__ == '__main__':
    ...
    if src is None:
        ...
        file_name = '../outputs/{}/models/{}/output_{}_{}.xlsx'.format(flag, tgt, args.n_owners, args.data_size)
        ...
        file_name = '../outputs/{}/models/{}2{}/output_{}_{}.xlsx'.format(flag, src, tgt, args.n_owners, args.data_size)
